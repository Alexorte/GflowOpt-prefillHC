...
from dag_gflownet.utils.metrics import expected_shd, expected_edges, threshold_metrics, posterior_score_stats, bic_score_of_dag, bdeu_score_of_dag
...
from dag_gflownet.hc_prefill import prefill_hc
from sklearn.model_selection import train_test_split
from dag_gflownet.utils.history import summarize_prefill_history
from dag_gflownet.utils.posterior_predictive import posterior_neg_avg_posterior_predictive_loglik
from dag_gflownet.utils.training_history import (
    should_compute_training_history,
    should_compute_slow_history_metrics,
    compute_training_history_row,
...
def main(args):
    ...
    # Reconstruir el scorer para que el entrenamiento use solo train
    scorer_name = scorer.__class__.__name__

    if scorer_name == 'BICScore':
        scorer = scorer.__class__(data_train)

    elif scorer_name == 'BDeScore':
        scorer = scorer.__class__(
            data=data_train,
            prior=scorer.prior,
            equivalent_sample_size=scorer.equivalent_sample_size
        )

    else:
        raise ValueError(f"No sé reconstruir el scorer de tipo {scorer_name} con data_train")
    ...
    # Entorno para prefill
    env_prefill = GFlowNetDAGEnv(
        num_envs=1,
        scorer=scorer,
        num_workers=args.num_workers,
        context=args.mp_context
    )
    # Entorno para el historial de entrenamiento
    history_env = GFlowNetDAGEnv(
        num_envs=args.num_envs,
        scorer=scorer,
        num_workers=args.num_workers,
        context=args.mp_context
    ...
    stats, history = prefill_hc(
        env=env_prefill, 
        replay_buffer=replay, 
        rng=rng, 
        num_episodes=args.prefill, 
        epsilon=args.hc_epsilon, 
        top_k=args.hc_top_k, 
        max_steps=args.hc_max_steps,
        history_every=args.history_every,
        history_callback=lambda stats, ep: summarize_prefill_history(stats, ep, graph, data_test)
    )
    hc_time = time.time() - hc_start

    args.output_folder.mkdir(parents=True, exist_ok=True)

    prefill_history_df = pd.DataFrame(history)
    prefill_history_df.to_csv(args.output_folder / 'prefill_history.csv', index=False)

    prefill_final_edges = np.array([s["final_num_edges"] for s in stats], dtype=float)
    prefill_final_scores = np.array([s["final_score"] for s in stats], dtype=float)
    prefill_final_dags = np.stack([s["final_adj"] for s in stats], axis=0)

    ground_truth_prefill = nx.to_numpy_array(graph, weight=None)

    prefill_shds = np.sum(np.abs(prefill_final_dags - ground_truth_prefill), axis=(1, 2))

    prefill_bic = np.array(
        [bic_score_of_dag(dag, data_test) for dag in prefill_final_dags],
        dtype=float
    )
    prefill_bdeu = np.array(
        [bdeu_score_of_dag(dag, data_test, equivalent_sample_size=1.0)
    ...
    with trange(start_iteration, total_iterations, desc='Training') as pbar:
        for iteration in pbar:
            ...
            should_update = iteration >= start_iteration
    ...
    mll_results = posterior_neg_avg_posterior_predictive_loglik(posterior, data_train, data_test, equivalent_sample_size=1.0)
    ...
    results = {
        ...
        **prefill_cache_summary,
        ...
        "Prefill_mean_shd": float(np.mean(prefill_shds)),
        "Prefill_best_shd": float(np.min(prefill_shds)),

        "Prefill_bic_mean": float(np.mean(prefill_bic)),
        "Prefill_bic_best": float(np.max(prefill_bic)),

        "Prefill_bdeu_mean": float(np.mean(prefill_bdeu)),
        "Prefill_bdeu_best": float(np.max(prefill_bdeu)),

        "hc_prefill_time_seconds": float(hc_time),
        "training_time_seconds": float(training_time),
        "evaluation_time_seconds": float(eval_time),
        "total_time_seconds": float(hc_time + training_time + eval_time),
    }
    ...
    save_training_history(args.output_folder, training_history)
...
if __name__ == '__main__':
    ...
    replay.add_argument('--prefill', type=int, default=1000,
        help='Number of episodes for pre-filling'
    ...
    misc.add_argument('--hc_epsilon', type=float, default=0.4,
        help='Epsilon de exploración para el prefill HC (default: %(default)s)')
    misc.add_argument('--hc_top_k', type=int, default=5,
        help='Número de mejores acciones entre las que explorar en el prefill HC (default: %(default)s)')
    misc.add_argument('--hc_max_steps', type=int, default=None,
        help='Máximo número de pasos por episodio en el prefill HC (default: sin límite)')
    misc.add_argument('--history_every', type=int, default=10,
        help='Frecuencia para guardar el historial del prefill HC (default: %(default)s)')
    misc.add_argument('--history_training', type=int, default=0,
        help='Frecuencia para guardar historial del entrenamiento GFlowNet; 0 lo desactiva')
    misc.add_argument('--num_samples_history_posterior', type=int, default=200,
        help='Número de muestras del posterior para checkpoints intermedios (default: %(default)s)')
    # 添加加载预训练模型相关参数
