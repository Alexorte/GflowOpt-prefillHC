...
class BDeScore(BaseScore):
    ...
    def state_counts(self, target, indices, indices_after=None):
        ...
        if len(parents) == 0:
            # Caso nodo raíz: una única configuración vacía de padres
            counts_series = data.groupby(variable, observed=False).size()
            state_count_data = counts_series.reindex(self.state_names[variable], fill_value=0).to_frame()

            # Nombre de columna dummy para representar la única configuración parental
            state_count_data.columns = [0]
            ...
            state_count_data = (data.groupby([variable] + parents, observed=False)
                                    .size()
                                    .unstack(parents))

            if isinstance(state_count_data, pd.Series):
                state_count_data = state_count_data.to_frame().T
